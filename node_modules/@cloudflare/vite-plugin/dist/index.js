...
var runDockerCmd = (dockerPath, args, stdio) => {
  ...
  const child = spawn2(dockerPath, args, {
    stdio: stdio ?? "inherit",
...
var runDockerCmdWithOutput = (dockerPath, args) => {
  try {
...
async function constructBuildCommand(options, logger) {
  ...
  const dockerfile = readFileSync3(options.pathToDockerfile, "utf-8");
  buildCmd.push("-f", "-");
...
function dockerBuild(dockerPath, options) {
  ...
  const child = spawn3(dockerPath, options.buildCmd, {
...
var resolve3 = async (options, resolver) => {
...
var getHeaders = async (config, options) => {
  const token = await resolve3(options, config.TOKEN);
  const username = await resolve3(options, config.USERNAME);
  const password = await resolve3(options, config.PASSWORD);
  const additionalHeaders = await resolve3(options, config.HEADERS);
  ...
  return containersConfig.map((container) => {
    if (isDockerfile(container.image, configPath)) {
      return {
        ...
        image_build_context: container.image_build_context ?? path7.dirname(container.image),
...
async function prepareContainerImages(options) {
  ...
  const uniqueImageTags = /* @__PURE__ */ new Set();
...
function getDebugPathHtml(workerNames, inspectorPort) {
  assert6(workerNames.length >= 1, "no workers present to debug");
  ...
  for (const worker of workers) {
    for (const value of Object.values(worker.durableObjects ?? {})) {
      if (typeof value === "string") {
        ...
        assert8(classNames, missingWorkerErrorMessage(worker.name));
        ...
        if (value.scriptName) {
          ...
          assert8(classNames, missingWorkerErrorMessage(value.scriptName));
...
async function getDevMiniflareOptions(config) {
  ...
  return {
    ...
    async unsafeModuleFallbackService(request2) {
      ...
      assert8(match, `Unexpected error: no match for module: ${rawSpecifier}.`);
...
function getPreviewModules(main, modulesRules) {
  assert8(modulesRules, `Unexpected error: 'modulesRules' is undefined`);
...
async function getPreviewMiniflareOptions(config) {
  const {
    resolvedPluginConfig,
    vitePreviewServer,
    inspectorPort,
...
`
      ...
      async configureServer(viteDevServer) {
        ...
        if (hasDevContainers) {
          ...
          entryWorkerConfig.dev.container_engine = resolveDockerHost(dockerPath);
        }
        ...
        if (resolvedPluginConfig.type === "workers") {
          ...
          if (staticRouting) {
            ...
            preMiddleware = async (req, res, next) => {
              assert12(req.url, `req.url not defined`);
    ...
    // Plugin that provides an __debug path for debugging the Cloudflare Workers.
    {
      ...
      async configurePreviewServer(vitePreviewServer) {
        ...
        const workerNames = resolvedPluginConfig.workers.map((worker) => {
          assert12(worker.name, "Expected the Worker to have a name");
          return worker.name;
        });
